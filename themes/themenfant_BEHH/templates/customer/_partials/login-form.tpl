...
<contact@prestashop.com>
  * @copyright Since 2007 PrestaShop SA and Contributors * @license
  ...
  {block name='login_form'} {block name='login_form_errors'} {include
  file='_partials/form-errors.tpl' errors=$errors['']} {/block}

  <form
    id="login-form"
    action="{block name='login_form_actionurl'}{$action}{/block}"
    method="post"
  >
    <div>
      {block name='login_form_fields'} {foreach from=$formFields item="field"}
      {block name='form_field'}
      <div class="form-group flex flex-col group">
        <label for="{$field.id}" class="text-black font-adlam text-base group-focus-within:text-be-bleu">{$field.label}</label>
        <input
          type="{$field.type}"
          name="{$field.name}"
      ...
      <div class="forgot-password">
        <a href="{$urls.pages.password}" rel="nofollow" class="text-be-rose hover:text-be-bleu transition ease-out duration-300">
    ...
    <footer class="form-footer text-sm-center clearfix">
      ...
      <button
        id="submit-login"
        class="btn btn-primary !bg-be-rose transition ease-out duration-500 hover:scale-[1.04] hover:!bg-be-bleu"
