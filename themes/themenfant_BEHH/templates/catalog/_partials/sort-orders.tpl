...
<contact@prestashop.com>
  * @copyright Since 2007 PrestaShop SA and Contributors * @license
  ...
  <span class="col-sm-3 col-md-5 hidden-sm-down sort-by text-black"
    >{l s='Sort by:' d='Shop.Theme.Global'}</span
  >
  <div
    class="{if !empty($listing.rendered_facets)}col-xs-8 col-sm-7{else}col-xs-12 col-sm-12{/if} col-md-9 products-sort-order dropdown"
  >
    <button
      class="btn-unstyle select-title text-black !border-3 !border-black !flex !justify-around !items-center"
      rel="nofollow"
      ...
      aria-expanded="false"
    >
      {if $listing.sort_selected}{$listing.sort_selected}{else}{l s='Choose'
