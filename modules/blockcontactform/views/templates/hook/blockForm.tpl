            ...
            <div class="cont_column_form">
                {if $settings['name_field']}
                    <div class="one_field_form one_field_line">
                        <span class="label_field_name">{l s='Name' mod='blockcontactform'}{if $settings['name_field_required']}<sup>*</sup>{/if}</span>
                        <input type="text" class="user_name">
                    </div>
                {/if}
                {if $settings['email_field']}
                    <div class="one_field_form one_field_line">
                        <span class="label_field_name">{l s='Email address' mod='blockcontactform'}{if $settings['email_field_required']}<sup>*</sup>{/if}</span>
                        <input type="text" class="user_email">
                    </div>
                {/if}
                {if $settings['phone_field']}
                    <div class="one_field_form one_field_line">
                        <span class="label_field_name">{l s='Phone Number' mod='blockcontactform'}{if $settings['phone_field_required']}<sup>*</sup>{/if}</span>
                        <input type="text" class="user_phone">
                    </div>
                {/if}
                {if $settings['subject_field']}
                    <div class="one_field_form one_field_line">
                        <span class="label_field_name">{l s='Subject Heading' mod='blockcontactform'}{if $settings['subject_field_required']}<sup>*</sup>{/if}</span>
                        <input type="text" class="subject_message">
                    </div>
                {/if}
                {if $settings['attach_field']}
                    <div  class="one_field_form one_field_form_attach">
                        <span class="label_field_name">{l s='Attach File' mod='blockcontactform'}</span>
                        <input type="file" name="fileUpload" class="filestyle"/>
                    </div>
                {/if}
                <div class="one_field_form one_field_line">
                    <span class="label_field_name">{l s='Message' mod='blockcontactform'}<sup>*</sup></span>
                ...
                <div class="one_field_form one_field_form_button" id="one_field_form_button">
                    <button class="send_contact_form_message btn btn-primary" onclick="">
                        <span>{l s='Send message' mod='blockcontactform'}</span>
                    </button>
